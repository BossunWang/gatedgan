...
def geometric_approximation(s):
    ...
    p = s.unsqueeze(-1) / s.unsqueeze(0) - I # lambda j / lambda i if i != j else is diagonal(p - I)
    p = th.where(p < 1., p, 1. / p) # keep ratio < 1. to lambda i > lambda j

    # input tensor([l1, l2, l3])
    # input.repeat(input.shape[0], 1)
    '''
    tensor([[l1, l2, l3],
            [l1, l2, l3]),
            [l1, l2, l3]])
    '''
    # a1 = input.repeat(input.shape[0], 1).t()
    '''
    tensor([[l1, l1, l1],
            [l2, l2, l2],
            [l3, l3, l3]])
    ...
    '''
    # a1_t = a1.t()
    # torch.where(a1 >= a1_t, a1, - a1_t) is symmetric matrix
    '''
    tensor([[l1, l1, l1],
            [-l1, l2, l2],
            [-l1, -l2, l3]])
    ...
    # 1/(li - lj) = -1/(lj - li) => neglect l3

    a1 = s.repeat(s.shape[0], 1).t() # repeat eigenvalue as column vector
    a1_t = a1.t() # transpose to row vector
    a1 = 1. / th.where(a1 >= a1_t, a1, - a1_t) # is equal to 1 / lambda i on eq8
